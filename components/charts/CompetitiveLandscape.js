...
export default function CompetitiveLandscape({ programs = [], initialVisible = 12 }) {
  ...
  const visible = expanded ? programs : programs.slice(0, initialVisible);
  ...
  return (
    <div className="clx">
      ...
      {programs.length > initialVisible ? (
        <button type="button" className="clx__more" onClick={() => setExpanded((e) => !e)}>
          {expanded ? "Show fewer programs" : `Show all ${programs.length} programs — including earlier-phase (${hidden} more) ▾`}
        </button>
      ) : null}
