  ...
  return (
    <>
      ...
      <Callout tone="info" title="Live from Drugs@FDA">
        <p>
          Application numbers, sponsors, and original drug-approval dates are pulled live from Drugs@FDA where available
          ...
          <table className="data">
            <thead>
              <tr>
                ...
                <th>Generic</th>
                <th>Sponsor</th>
                <th className="nowrap">Original approval</th>
      ...
      <section className="section">
        ...
        <Callout tone="info" title="Latest FDA labels, side by side">
          <p>Most recent label per drug, with indication and dosing sections. Click <em>Show more</em> in any cell to expand the text inline, or open the full current label on DailyMed.</p>
        </Callout>
        <div style={{ height: 12 }} />
        {!labels.ok ? (
              ...
              <tbody>
                {labels.items.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td className="muted">{l.indications ? <ExpandableText text={l.indications} limit={180} /> : (l.hasLabel ? "—" : "No FDA label indexed")}</td>
                    <td className="muted">{l.dosage ? <ExpandableText text={l.dosage} limit={160} /> : "—"}</td>
