      ...
      {topTherapies.length === 0 ? (
        ...
        topTherapies.map((d) => (
          <div className="brief__entry" key={d.id}>
            ...
            {d.curated ? <span className="badge badge--preprint" style={{ marginLeft: 8 }}>Curated</span> : null}
            <div className="feed__meta">{d.manufacturer} · {formatDate(d.effectiveDate)}</div>
          </div>
        ))
      )}
