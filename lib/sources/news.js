...
async function fetchFeed(feed, revalidate = REVALIDATE_SECONDS) {
...
export async function getNews(area, limit = 24, { revalidate = REVALIDATE_SECONDS } = {}) {
  const accessDate = new Date().toISOString();
  try {
    const settled = await Promise.allSettled((area.newsFeeds || []).map((f) => fetchFeed(f, revalidate)));
    const merged = [];
    const seen = new Set();
    let anyOk = false;
