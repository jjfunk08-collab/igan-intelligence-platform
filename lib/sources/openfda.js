...
export async function getApprovedTherapies(area) {
  ...
  try {
    ...
    const items = known.map((entry, i) => {
      ...
      const live = !!daf;
      const dafSearch = `https://www.accessdata.fda.gov/scripts/cder/daf/index.cfm?event=BasicSearch.process&searchTerm=${encodeURIComponent(entry.brand)}`;
      const eparUrl = `https://www.ema.europa.eu/en/medicines?search_api_fulltext=${encodeURIComponent(entry.generic || entry.brand)}`;
      return {
        ...
        drugsFdaUrl: (daf && daf.url) || dafSearch,
        eparUrl,
        live,
        url: (daf && daf.url) || dafSearch,
      };
    });
    ...
    const items = known.map((entry) => {
      ...
      return {
        ...
        applNo: null, applType: null, marketingStatus: null,
        drugsFdaUrl: dafSearch, eparUrl,
        live: false,
        url: entry.url || dafSearch,
      };
    });
    return {
