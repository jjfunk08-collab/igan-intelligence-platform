...
function normalize(study) {
  ...
  const conditions = (pick(p, ["conditionsModule", "conditions"], []) || []).filter(Boolean);
  ...
  const pcStruct = pick(p, ["statusModule", "primaryCompletionDateStruct"], {}) || {};
  const primaryCompletion = pcStruct.date || null;
  const primaryCompletionType = pcStruct.type || null; // ESTIMATED | ACTUAL
  const completionStruct = pick(p, ["statusModule", "completionDateStruct"], {}) || {};
  const completion = completionStruct.date || null;
  ...
  return {
    ...
    primaryCompletionDate: primaryCompletion || null,
    primaryCompletionType: primaryCompletionType, // ESTIMATED (projected) | ACTUAL
    ...
    lastUpdateType: pick(p, ["statusModule", "lastUpdatePostDateStruct", "type"], null),
    historyUrl: nctId ? `https://clinicaltrials.gov/study/${nctId}?tab=history` : null,
