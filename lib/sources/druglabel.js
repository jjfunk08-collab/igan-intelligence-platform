...
// Fuller cleaned text (joins all segments, caps to keep payload sane) so the
// UI can show a preview and expand to the rest without leaving the page.
function fullText(arr, cap = 6000) {
  const t = Array.isArray(arr) ? arr.join("\n\n") : arr;
  if (!t) return null;
  const clean = String(t).replace(/[ \t]+/g, " ").replace(/\n{3,}/g, "\n\n").trim();
  return clean.length > cap ? clean.slice(0, clean.lastIndexOf(" ", cap)).trim() + "…" : clean;
}
...
async function fetchLabel(brand, keyParam) {
  ...
  try {
    ...
    return {
      ...
      indications: fullText(rec.indications_and_usage),
      dosage: fullText(rec.dosage_and_administration),
      url: setid
