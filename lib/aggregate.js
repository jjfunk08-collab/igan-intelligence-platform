...
// Indications/conditions under study across the competitor trial set — a tally
// of the conditions ClinicalTrials.gov records list, so users can see which
// sub-indications the field is actually pursuing.
export function indicationsFromTrials(trials = [], limit = 12) {
  const m = new Map();
  for (const t of trials) {
    for (const cRaw of t.conditions || []) {
      const c = String(cRaw).trim();
      if (!c) continue;
      const key = c.toLowerCase();
      if (!m.has(key)) m.set(key, { condition: c, count: 0 });
      m.get(key).count += 1;
    }
  }
...
// Projected readouts: trials whose PRIMARY completion date is an ESTIMATE in
// the future — i.e., an upcoming, not-yet-reached readout. Sorted soonest
// first. Carries the primary endpoint so users see what will be read out.
export function projectedReadouts(trials = [], limit = 12) {
  const now = Date.now();
  return trials
    .filter((t) => t.primaryCompletionDate && /EST/i.test(t.primaryCompletionType || ""))
    .map((t) => ({ t, ms: new Date(t.primaryCompletionDate).getTime() }))
...
// Recently changed trial records, newest change first (by last-update-posted
// date). Each carries a historyUrl to the authoritative change log.
export function recentlyUpdated(trials = [], limit = 12) {
  return trials
